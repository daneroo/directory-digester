...
time deno run ${DENO_PERMS} ${REPO_SRC} --verbose ${BENCH_DIRECTORY}

## Docker deno (galois, davinci)

docker pull --quiet ${DOCKER_DENO_TAG}
time docker run --rm -it ${MACOS_VOLS} ${DOCKER_DENO_TAG} run ${DENO_PERMS} ${REPO_SRC} --verbose ${BENCH_DIRECTORY}

## Docker deno (syno)

docker pull --quiet ${DOCKER_DENO_TAG}
time -f %es docker run --rm -it ${SYNO_VOLS} ${DOCKER_DENO_TAG} run ${DENO_PERMS} ${REPO_SRC} --verbose ${BENCH_DIRECTORY}
